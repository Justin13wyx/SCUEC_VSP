(function () {
		...
		control_btns[i].addEventListener("click", e => {
			...
			if ( action == "cancel" )
				...
				if (confirm("确定设置成" + admin_input.children.pass.value + "?"))
					fetch_data(true, "POST", "https://dygx.scuec.edu.cn:5000/apiv1/admin/setpass", check_setpass_state, data)
			}
		})
	}
	...
	function _fetch_video(res) {
		...
		for (let i = 0; i < videos.length; i ++) {
			let video = videos[i]
			html += `<li class="" link=https://dygx.scuec.edu.cn:5000/${video[1]}><span class="seen_token">○</span>${video[0]}</li>`
		}
		html += "</ul>"
		...
		if ( admin_state == "users" ) {
			...
			if ( action == "del" ) {
				...
				if ( confirm("确定删除下面的项目吗?\n" + target) ) {
					...
					data = make_data(data)
					fetch_data(true, "POST", "https://dygx.scuec.edu.cn:5000/apiv1/admin/del", check_del_state, data)
				}
				else
					return;
			}
	...
	function do_user_action(target, action) {
		...
		if ( action == "search" ) {
			...
			if (filterSqlStr(username) && validate(username)) {
				...
				data.set("state", admin_state)
				data = make_data(data)
				fetch_data(true, "POST", "https://dygx.scuec.edu.cn:5000/apiv1/admin/searchUser", re_renderuser, data)
			}
			return;
		}
		let data = new Map()
...
这是选项B, 本题只有两个选项`
	...
	function render_result(res) {
		...
		result_context.fillText("中南民族大学大型仪器开放共享服务平台测评报告", result_canvas.width * 0.05, canvas.height * 0.2)
		result_context.fill()
		result_context.restore()
		// 填充信息
		result_context.save()
