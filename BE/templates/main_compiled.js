	...
	for (var _i = 0; _i < control_btns.length; _i++) {
		control_btns[_i].addEventListener("click", function (e) {
			...
			if (action == "cancel") toggle_admin_inputbox(false);else {
				...
				if (confirm("确定设置成" + admin_input.children.pass.value + "?")) fetch_data(true, "POST", "https://dygx.scuec.edu.cn/te/apiv1/admin/setpass", check_setpass_state, _data);
			}
		});
	}
  ...
  */
	function _fetch_video(res) {
		...
		for (var _i11 = 0; _i11 < videos.length; _i11++) {
			...
			html += "<li class=\"\" link=https://dygx.scuec.edu.cn/te/" + _video[1] + "><span class=\"seen_token\">\u25CB</span>" + _video[0] + "</li>";
		}
		html += "</ul>";
			...
			do_user_action(target, action);
				...
				if (action == "del") {
					...
					if (confirm("确定删除下面的项目吗?\n" + target)) {
						...
						fetch_data(true, "POST", "https://dygx.scuec.edu.cn/te/apiv1/admin/del", check_del_state, _data2);
					} else return;
				}
			}
	}
	...
	function do_user_action(target, action) {
		...
		if (action == "search") {
			...
			if (filterSqlStr(_username) && validate(_username)) {
				...
				fetch_data(true, "POST", "https://dygx.scuec.edu.cn/te/apiv1/admin/searchUser", re_renderuser, _data3);
			}
			return;
		}
		var data = new Map();
