		...
		control_btns[i].addEventListener("click", e => {
			...
			if ( action == "cancel" )
				...
				if (confirm("确定设置成" + admin_input.children.pass.value + "?"))
					fetch_data(true, "POST", "http://127.0.0.1:5000/apiv1/admin/setpass", check_setpass_state, data)
			}
		})
	}
		...
		if ( admin_state == "users" ) {
			...
			if ( action == "del" ) {
				...
				if ( confirm("确定删除下面的项目吗?\n" + target) ) {
					...
					data = make_data(data)
					fetch_data(true, "POST", "http://127.0.0.1:5000/apiv1/admin/del", check_del_state, data)
				}
				else
					return;
			}
